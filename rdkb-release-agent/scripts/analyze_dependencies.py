...
parser.add_argument("--auto-resolve", action="store_true",
                    help="If set, auto-include dependency PRs in the output")
...
if llm and findings:
    ...
    for f in findings:
        ...
        is_dependent = llm.evaluate_dependency(diff_a, diff_b)
        
        if is_dependent:
            print(c(GREEN, "YES (Dependent)"))
...
if AUTO_RESOLVE:
    ...
    for dep_num in dep_prs:
        if dep_num not in INCLUDE_NUMS:
...
if not findings:
    ...
    if AUTO_RESOLVE and auto_added:
        print(f"  {ok(f'Auto-included dependency PRs: {auto_added}')}")
        print(f"  {dim('These will be cherry-picked BEFORE the PRs that depend on them.')}")
    else:
        print(f"  {warn('Set auto_resolve_deps: true in .release-config.yml to auto-include these.')}")
