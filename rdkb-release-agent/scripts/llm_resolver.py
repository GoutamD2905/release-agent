...
class LLMResolver:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(f"LLM API key is empty. Set the environment variable.")

        if self.provider not in ("openai", "gemini", "githubcopilot", "azureopenai"):
            raise ValueError(f"Unsupported LLM provider: {self.provider}. Use 'openai', 'gemini', 'githubcopilot', or 'azureopenai'.")
            
        if self.provider == "azureopenai" and not self.endpoint:
            raise ValueError(f"Azure OpenAI requires an 'endpoint' setting in the config.")

    def evaluate_dependency(self, diff_a: str, diff_b: str) -> bool:
        """
        ...
        if self._call_count >= self.max_calls:
            ...
            return True # Fallback to True (safe assumption)
        ...
        user_prompt = (
            ...
            "If there is a true dependency, reply exactly with: YES\n"
            "IMPORTANT: Reply ONLY with 'YES' or 'NO'."
        )
    ...
    def resolve_conflict(
        ...
        pr_context_str = ""
        if self.release_plan:
            strategy = self.release_plan.get("strategy", "unknown")
            prs = self.release_plan.get("operation_prs", [])
            if prs:
    ...
    def _log_feedback(
        ...
        entry = {
            ...
            "resolved_lines": len(resolved) if resolved else 0,
            "ours_preview": ''.join(ours[:5])[:200],
            "theirs_preview": ''.join(theirs[:5])[:200],
            "resolved_preview": ''.join(resolved[:5])[:200] if resolved else "",
        }
        try:
